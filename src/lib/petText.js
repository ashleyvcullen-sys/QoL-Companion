...
// Fills {name}, {they}, {their}, {them}, {are}, {have}, {s} and their
// capitalised forms ({They}, {Their}). Leaves anything it doesn't recognise
...
export function fillPetText(template, pet) {
  if (!template) return ''
  const tokens = { name: pet?.name ?? 'your pet', ...petPronouns(pet) }
